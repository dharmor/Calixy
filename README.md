...
The startup edition also seeds a timezone catalog into SQLite from a local snapshot `Timezones.php` so the selected system timezone is stored in the database instead of only in config.
